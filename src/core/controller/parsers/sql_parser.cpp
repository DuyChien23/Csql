...
namespace Csql {
    ...
    Statement* SqlParser::createEntityStatement(Tokenizer *aTokenizer) {
        ...
        while (!aTokenizer->currentIsChar(')')) {
            if (aTokenizer->currentIs(SqlKeywords::foreign_kw)) {
                ...
                aTokenizer->check(SqlKeywords::foreign_kw)->check(SqlKeywords::key_kw)->check("(")->consumeType(attributeName)->check(")");
                std::string refEntityName, refAttributeName;
                aTokenizer->check(SqlKeywords::references_kw)->consumeType(refEntityName)->check("(")->consumeType(refAttributeName)->check(")");
    ...
    bool makeJoinExpression(Tokenizer *aTokenizer, ExpressionParser& expressionParser, SQLQueryPtr &theQuery) {
        if (aTokenizer->currentIs(SqlKeywords::inner_kw) || aTokenizer->currentIs(SqlKeywords::left_kw) || aTokenizer->currentIs(SqlKeywords::right_kw)) {
            SqlKeywords joinType;
            std::string entityName;
            aTokenizer->consumeType(joinType)->check(SqlKeywords::join_kw)->consumeType(entityName)->check(SqlKeywords::on_kw);
