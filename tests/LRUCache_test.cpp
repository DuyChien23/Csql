...
#include "../src/Core/Storage/LRUCache/lru_cache.h"

Csql::LRUCache<int, int*>* initStandardLRUCache(size_t maxSize) {
    Csql::LRUCache<int, int*>* lru_cache = new Csql::LRUCache<int, int*>(maxSize);
    return lru_cache;
}

TEST(LRU_CACHE_TEST, PUT_GET_METHOD_TEST) {
    size_t maxSize = 2;
    Csql::LRUCache<int, int*>* lru_cache = initStandardLRUCache(maxSize);
